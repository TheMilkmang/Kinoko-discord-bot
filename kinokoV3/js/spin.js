...
function makeWheel(width, height, choices){
	return new Promise(function(resolve, reject){
		var canvas = new Canvas(width, height);
		var ctx = canvas.getContext('2d');
		var wheel = new Image();
...
exports.spinChoose = function(message){
	...
	loadImg(url).then( img => {
		...
		makeSpinner(img).then( spinner => {
			...
			makeWheel(256, 256, choices).then( wheel => {
...
var prizes = [0, 0, 0, 0, 0, 0.25, 0.25, 0.5, 0.5, 1.25, 1.25, 1.5, 1.5, 2, 2, 'JP']
...
exports.betSpin = function(message){
	...
	if( args[1].endsWith('m') || args[1].endsWith('p') ){
		...
		if(bet <= 99 || bet > 10000){
			message.channel.send("Please place a bet between 100 and 10,000")
			return;
		}

		if(currency == 'm'){
			mushroomSpin(message.channel, message.author, bet);
		}else{
			return;
...
function mushroomSpin(channel, user, bet){
	if(bank.subtractItemUser(user, 'mushrooms', bet)){
		...
		loadImg(url).then( img => {
			...
			makeSpinner(img).then( spinner => {
				...
				makeWheel(256, 256, prizeShuffle).then( wheel => {
					...
					if(spinned.choice == 'JP'){
						var pay = kinoko.mushroomSpin.jp + bet;
						kinoko.mushroomSpin.jp = 0;
					}else{
					...
					channel.send(endTime - startTime + 'ms. ' + user + spinned.choice + 'x You won ' + pay + ':mushroom:! The jackpot is now ' + kinoko.mushroomSpin.jp + ':mushroom:', attachment);
				});
