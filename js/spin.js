...
var prizes = [0, 0, 0, 0, 0, 0, 0, 0, 0.5, 1.25, 1.25, 1.5, 1.5, 2, 2, 6]
		...
		loadImg(url).then( img => {
			...
			makeSpinner(80, 80, img).then( spinner => {
				...
				makeWheel(200, 200, 12, prizeShuffle).then( wheel => {
					...
					kinoko.mushroomSpin.income += bet;
					kinoko.mushroomSpin.outcome += pay;
					bank.addItemUser(user, 'mushrooms', pay);
					
					if(all == false){
						channel.send(endTime - startTime + 'ms. ' + user.username + ' ' + spinned.choice + 'x You risked ' + bet + ' and won ' + pay + ':mushroom:!', attachment);
					}else{
						channel.send(endTime - startTime + 'ms. ' + user.username + ' ' + spinned.choice + 'x You risked all ' + bet + ':mushroom: for a chance at double the payout and won ' + pay + ':mushroom:!', attachment);
					}
				});
