...
exports.slideText = function(message){ //]slide <text>
	...
	for(var i = 0; i < frames; i++){
		...
		ctx.fillText(msg, txtWidth - i*3, height/2);
		encoder.addFrame(ctx);
	}
	encoder.finish();
	var d = new Date();
...
exports.waveText = function(message){ //]wave <text>
	var msg = message.content.slice(6);
	var msgArray = msg.split('');
	var width = 250;
	var height = 30;
	var measureCanv = new Canvas(1000, 1000);
	var mctx = measureCanv.getContext('2d');
	mctx.font =  "14px Whitney Light";
	var txtWidth = mctx.measureText(msg).width;
	//var width = Math.min(txtWidth*2, 500);
	...
	ctx.font =  "14px Whitney Light";
	//canvas.width = 200;
	for(var i = 0; i < frames; i++){
		ctx.clearRect(0, 0, canvas.width, canvas.height);
		for(var j = 0; j < msgArray.length; j++){
			var y = height/2 + Math.round(Math.sin(( (j + i) * 1.25) * 0.25 )*5);
			var x = j * 10;
			ctx.fillText(msgArray[j], x , y);
		}
